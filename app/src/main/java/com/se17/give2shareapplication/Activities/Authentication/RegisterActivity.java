...
public class RegisterActivity extends AppCompatActivity {
    ...
    private boolean validate() {
        ...
        if (txtCnic.getText().toString().isEmpty()) {
            layoutCnic.setErrorEnabled(true);
            layoutCnic.setError("CNIC Number is required");
            return false;
        }

        if (txtCnic.getText().toString().length()<13 || txtCnic.getText().toString().length()>13) {
            layoutCnic.setErrorEnabled(true);
            layoutCnic.setError("Enter Valid CNIC Number");
            return false;
        }

        if (txtPhone.getText().toString().length()<11 || txtPhone.getText().toString().length()>11) {
            layoutPhone.setErrorEnabled(true);
